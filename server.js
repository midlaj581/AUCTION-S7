...
let config = {
  ...
  // Bid increment rules:
  thresholdBid:      200,   // below this → highIncrement, at/above → lowIncrement
  highIncrement:     20,
  lowIncrement:      10,
};
...
function broadcastState() {
  io.emit('stateUpdate', { auctionState, teams, players, config });
}

// ─── REST ─────────────────────────────────────────────────────────────────────
app.get('/api/state', (req, res) => res.json({ auctionState, teams, players, config }));

// ─── SOCKET ───────────────────────────────────────────────────────────────────
io.on('connection', (socket) => {
  socket.emit('stateUpdate', { auctionState, teams, players, config });
  ...
  socket.on('admin:sold', () => {
    ...
    player.status = 'sold'; player.soldTo = team.name; player.soldPrice = price;
    team.spent += price;
